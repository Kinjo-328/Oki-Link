...
class OkiLinkSupabaseUser extends BaseAuthUser {
  OkiLinkSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> okiLinkSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = OkiLinkSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
